...
try {
  ...
  // ⚠︎ 1040 HIGH, NOT THE 820 SOME SIBLINGS USED. This kit's payoff is the ROUTING panel under
  // the field table -- ten field rows plus three routed rows -- and at 820 the 'Needs an ops
  // follow-up' row falls below the fold, so the published shot would show everything except the
  // thing the kit decides. A sibling kit shipped exactly that crop once, found by opening the
  // PNG rather than by any check; this one starts at the taller viewport.
  ...
  if (LIVE) {
    // POD-0026 is the record that exercises all three moving parts at once, which is why it is
    // the shot rather than an alphabetical first pick: 240 units ordered, 222 scanned at the
    // drop -- eighteen short -- an undamaged condition code, NO recipient signature on file,
    // and a driver note reading "All good on my end. Left with the receiving clerk, no issues
    // to report." The prose says fine; the counts say eighteen units never arrived; and because
    // nobody signed for it, the pure-code rule routes it to the ops desk. The tone floor in
    // evals/baseline.py calls it complete -- a false negative in results/eval-b000-rules.json.
    await page.select('#doc', 'POD-0026')
    await sleep(300)
  }
