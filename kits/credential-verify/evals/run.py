...
Usage:
    python -m evals.run --run-id r001-<model>            # the real run, one call per file
    python -m evals.run --run-id t000 --stub             # no key, no spend, proves the wiring
    python -m evals.run --run-id b000-rules --baseline    # free, keyword floor, no model
"""
import argparse
import json
import os
...
def stub_complete(cfg, system, user, max_tokens=1024):
    ...
    out["provider_name"] = m.group(1).strip() if m else None
    m = re.search(r"NPI\n-+\n(.+)", user)
    out["npi"] = m.group(1).strip() if m else None
...
def main():
    ...
    o = scored["overall"]
    print("\n%-22s %s" % ("run", a.run_id))
    print("%-22s %d/%d  (%s)" % ("extraction accuracy", ext_hit_for_print(scored),
                                 o["extraction_cells"], o["extraction_accuracy"]))
    print("%-22s %s" % ("review-flag recall", flag_scored["recall"]))
    print("%-22s %d" % ("false negatives", flag_scored["false_negative"]))
    print("-> %s" % os.path.relpath(path, HERE))


def ext_hit_for_print(scored):
    return sum(1 for c in scored["cells"] if c["verdict"] == "hit")
