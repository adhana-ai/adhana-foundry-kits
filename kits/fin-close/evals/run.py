"""Run the checker over the corpus and write one result file. THIS SPENDS MONEY.

Usage:
    python -m evals.run --run-id r001-<model>             # the real run, ONE call per close cycle
...
⚠︎ ONE CALL PER CLOSE CYCLE, COUNTED BEFORE IT IS MADE — same discipline as every sibling kit's
...
from src.close import MAX_TOKENS                    # noqa: E402
...
def stub_complete(cfg, system, user, max_tokens=1024, **kw):
    """A deterministic fake provider. It answers `clean` to every check and cites the basis
    document's first line for it. That is an honestly mediocre checker — it exercises prompt ->
    call -> parse -> score end to end without a key. It is NOT a baseline; evals/baseline.py is."""
    marker = "BASIS DOCUMENT\n--------------\n"
...
def main():
    ...
    print("cycles   : %d   checks per cycle: %d   total checks: %d"
          % (len(cycles), len(P.CHECKS), len(cycles) * len(P.CHECKS)))
    print("calls    : %d  (one per close cycle — all %d checks are batched)"
          % (len(cycles), len(P.CHECKS)))
    ...
    for i, cycle in enumerate(cycles, 1):
        basis_text = C.load_basis(cycle["rje_id"])
        t0 = time.time()
        r = C.check(cfg, cycle, basis_text, complete=complete, thinking=thinking,
                   prompt=a.prompt)
        ...
        if first is None and r["parsed"]:
            first = r
        rec = {"close_id": cycle["close_id"], "rje_id": cycle["rje_id"],
        ...
        print("  %2d/%d  %-10s %d/%d answered  %s"
              % (i, len(cycles), cycle["close_id"], r["answered"], r["asked"],
    ...
    for c, pc in scored["per_check"].items():
        print("  %-10s accuracy %s%%  (gold %s)" % (c, pc["accuracy_pct"], pc["gold_total"]))
