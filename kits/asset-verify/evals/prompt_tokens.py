"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001 --docs 1
...
`prompt_tokens` counts the provider itself returns. That is the provider's own tokenizer measuring
its own prompt, the number that matches an invoice.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields):
    ...
    return system, [("", "chat protocol and system prompt"),
                    (a, "field schema"),
                    (full_user, "statement sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="AV-0001")
