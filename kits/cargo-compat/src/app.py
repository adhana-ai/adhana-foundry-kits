...
ONE PRE-LOAD CHECK SHEET, ONE EXTRACTION, on your machine, with your key. It is the proof that this
runs on a laptop; it is NOT the published dashboard, which lives on the Foundry site from committed
run records.

IT RENDERS WITH NO KEY. /api/fields, /api/doc and /api/matrix need nothing; only /api/extract calls
a provider, and with no API_KEY it returns 200 saying so rather than an error.

⚑ THE MATRIX IS SERVED TO THE PAGE, NOT DESCRIBED TO IT. /api/matrix returns data/matrix.json
verbatim, so the table the verdict was decided by is one click away from the verdict itself -- and
the "illustrative, not an authority" line the reader needs is on the same screen as the answer
rather than in a file they will never open.
"""
import argparse
import errno
import json
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8797 was free when this kit was written;
# `lsof -nP -iTCP:8797 -sTCP:LISTEN` says whether it still is, and --port moves it either way.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                ...
                                "required_wash": r["recomputed_required_wash"],
...
def main():
    ...
    print("cargo-compat UI  ->  http://127.0.0.1:%d" % a.port)
    print("  sheets: %d   fields: %d   API_KEY: %s"
          % (len(EX.documents()), len(EX.load_fields()),
             "set" if config.has_key(cfg) else "not set (the page still renders)"))
    print("  the shipped matrix is ILLUSTRATIVE and is not an authority; this kit proposes a "
          "verdict and never authorises a load")
    srv.serve_forever()
