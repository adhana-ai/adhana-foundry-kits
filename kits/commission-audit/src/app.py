...
ONE CHANNEL COMMISSION CLAIM RECORD, ONE EXTRACTION, on your machine, with your key. It is the
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. This one deliberately skips a few past
# the last taken number rather than claiming the next one: it was written alongside several other
# kits at the same time, and "the next free port" is the same answer every one of them would
# reach for.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                ...
                                "recomputed_owed_usd": r["recomputed_owed_usd"],
...
def main():
    ...
    print("commission-audit UI  ->  http://127.0.0.1:%d" % a.port)
