...
Usage:
    ...
    python -m evals.run --run-id r001-exception-brief                 # THE REAL RUN, one call/batch
    python -m evals.run --run-id r001-x --limit 5                      # a costed toe in the water

⚠︎ ONE CALL PER BATCH, COUNTED BEFORE IT IS MADE -- same discipline as every sibling kit's run.py.
...
def stub_complete(cfg, system, user, max_tokens=1024, **kw):
    ...
    unreliable_evidence as false for every item. Not a baseline -- evals/baseline.py is; this only
    exercises prompt -> call -> parse -> score end to end with no key."""
    ids = _ID_RE.findall(user)
    items = [{"item_id": i, "cause": "unknown", "citation_1": "", "citation_2": "",
             "unreliable_evidence": False, "note": "stub: exercising the pipeline only"} for i in ids]
    body = {"items": items, "narrative": "Stub narrative exercising the pipeline only."}
...
def main():
    ...
    for i, batch in enumerate(batches, 1):
        bid = batch["batch_id"]
        bnotes = notes.get(bid, [])
        t0 = time.time()
        r = B.draft(cfg, batch, bnotes, complete=complete, thinking=thinking, prompt=prompt_v)
        ms = int((time.time() - t0) * 1000)
        lat.append(ms)
        tin += r.get("input_tokens") or 0
        tout += r.get("output_tokens") or 0
        if not r["answer"]["items"] and r["items_material"] > 0:
            no_answer += 1
        if first is None and r["answer"]["items"]:
            first = r
        records.append({
            "batch_id": bid, "packed": r["packed"], "pack_meta": r["pack_meta"],
            "items_material": r["items_material"], "items_answered": r["items_answered"],
            "answer": r["answer"], "finish_reason": r.get("finish_reason"),
            "input_tokens": r.get("input_tokens"), "output_tokens": r.get("output_tokens"),
            "reasoning_tokens": r.get("reasoning_tokens"), "latency_ms": ms,
            "raw_text": r.get("raw_text", "") if r["items_answered"] < r["items_material"] else None,
        })
        print("  %2d/%d  %-10s %d/%d items answered"
              % (i, len(batches), bid, r["items_answered"], r["items_material"]))
    ...
    out = {
        ...
        "batches": len(records), "no_answer_batches": no_answer,
