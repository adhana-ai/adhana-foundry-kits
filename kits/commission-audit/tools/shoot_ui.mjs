...
try {
  ...
  // ⚠︎ 1240 HIGH, TALLER THAN THE SIBLING KIT THIS WAS COPIED FROM. This kit's payoff is the
  // ROUTING panel under the field table -- FOURTEEN field rows plus four routed rows -- and at
  // 1040 the 'Needs a recovery claim' row falls below the fold.
  ...
  if (LIVE) {
    // CMA-0017 exercises every moving part at once: a no-show booking whose guest never
    // arrived, a cancellation penalty of 348.22 USD actually charged, a 15.0 pct contracted rate
    // and a claimed commission of 52.23 USD -- which is exactly 15 pct OF THE PENALTY, so the
    // claim is valid even though nobody stayed. The reviewer note reads "Something looked off
    // against the folio on this line, revisit before settlement." (the wrong register -- this
    // claim IS owed), and the invoice is already paid, so a reader who called it invalid would
    // also fire the recovery flag on it. The free floor does exactly that.
    await page.select('#doc', 'CMA-0017')
    await sleep(300)
  }
