...
Usage:
    python -m evals.run --run-id r001-<model>            # the real run, one call per stub
...
def stub_complete(cfg, system, user, max_tokens=1024):
    import re
    out = {}
    m = re.search(r"Employee\n-+\n(.+)", user)
    out["employee_name"] = m.group(1).strip() if m else None
    m = re.search(r"Employer\n-+\n(.+)", user)
    out["employer_name"] = m.group(1).strip() if m else None
...
def main():
    ...
    for i, stmt_id in enumerate(docs, 1):
        ...
        incomes[stmt_id] = r.get("qualifying_monthly_income")
        if first is None:
            first = r
    ...
    print("%-22s %s" % ("needs-review recall", flag_scored["recall"]))
