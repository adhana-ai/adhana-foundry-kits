...
Usage:
    ...
    python -m evals.run --run-id b000-rules --baseline   # free, severity-register floor, no model
"""
import argparse
import json
import os
...
def stub_complete(cfg, system, user, max_tokens=1024):
    import re
    out = {}
    m = re.search(r"Case ID\n-+\n(.+)", user)
    out["case_id"] = m.group(1).strip() if m else None
    m = re.search(r"Suspect Drug\n-+\n(.+)", user)
    out["suspect_drug"] = m.group(1).strip() if m else None
...
def main():
    ...
    serious_scored = J.score_seriousness(records, golds)
    ...
    out = {
        ...
        "seriousness_scores": serious_scored,
    ...
    print("\n%-26s %s" % ("run", a.run_id))
    print("%-26s %d/%d  (%s)" % ("extraction accuracy", ext_hit, o["extraction_cells"],
                                 o["extraction_accuracy"]))
    print("%-26s %s" % ("is_serious accuracy", serious_scored["overall"]["accuracy"]))
    for reg in sorted(serious_scored["by_register"]):
        d = serious_scored["by_register"][reg]
        print("   %-23s %d/%d  (%s)" % (reg, d["correct"], d["n"], d["accuracy"]))
    print("%-26s %s" % ("review-flag recall", flag_scored["recall"]))
    print("%-26s %s" % ("review-flag precision", flag_scored["precision"]))
    print("%-26s %d" % ("false negatives", flag_scored["false_negative"]))
