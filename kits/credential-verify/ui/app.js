...
function drawComputed(flag, reasons) {
  $("c-flag").textContent = (flag === null || flag === undefined)
    ? "not computed" : (flag ? "YES — route to credentialing committee" : "no");
  $("c-reasons").textContent = (reasons && reasons.length) ? reasons.join(", ") : "—";
}
...
async function go() {
  ...
  try {
    ...
    draw(r.fields);
    drawComputed(r.needs_review, r.review_reasons);
