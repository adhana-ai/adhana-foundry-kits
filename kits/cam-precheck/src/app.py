...
ONE CAM RECONCILIATION LINE, ONE PRE-CHECK, on your machine, with your key. It is the proof that
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8844 rather than the next free number in
# sequence: this kit was built alongside eight others in one afternoon, all of them claiming ports,
# so it takes the one matching its own use-case number instead of racing for 8793.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                ...
                                "recomputed_permitted_usd": r["recomputed_permitted_usd"],
                                "recomputed_line_ok": r["recomputed_line_ok"],
...
def main():
    ...
    print("cam-precheck UI  ->  http://127.0.0.1:%d" % a.port)
    print("  lines: %d   fields: %d   API_KEY: %s"
