"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-order-dates --doc ORD-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. Counting a court deadline
needs a definition of a business day and a list of the days the court is shut, so the rulebook has
to be in the call -- which makes "how much of every call is the holiday list" a question a reader
should be able to answer with a number rather than a guess.
"""
import argparse
import json
import os
