...
function draw(fields) {
  ...
  // Every field is stated on every record in this corpus, so a blank here is a real miss —
  // the label still says "not stated" rather than "not found", because what the panel knows is
...
function drawComputed(complete, signature, flag) {
  $("c-complete").textContent = complete == null ? "—" : complete;
  $("c-sig").textContent = signature == null ? "—" : signature;
  $("c-flag").textContent = (flag === null || flag === undefined)
    ? "not computed — one of the two values the rule needs was missing"
    : (flag ? "YES — incomplete and unsigned, route it to the ops desk"
...
async function go() {
  ...
  try {
    ...
    drawComputed(val("delivery_complete"), val("recipient_signature_present"), r.needs_review);
