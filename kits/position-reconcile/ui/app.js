...
function drawComputed(breakQty, flag) {
  $("c-qty").textContent = (breakQty === null || breakQty === undefined)
    ? "not computed — a required input was missing" : breakQty;
  $("c-flag").textContent = (flag === null || flag === undefined)
    ? "not computed" : (flag ? "YES — route to operations supervisor" : "no");
}
...
async function go() {
  ...
  try {
    ...
    draw(r.fields);
    drawComputed(r.break_quantity, r.needs_review);
