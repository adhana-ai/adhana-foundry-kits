"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-permit-obligations --doc REG-0007 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. The model does not compute
a status -- code does -- so "how much of every call is a rulebook the model never applies" is a
question a reader should be able to answer with a number rather than a guess, and it is the one
piece of this prompt whose value is genuinely unmeasured.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields, ob_fields):
    secs = segment.sections(doc_text)
    msgs, parts, _ = P.build(doc_text, secs, fields, ob_fields, selector)
    system, full_user = msgs[0]["content"], msgs[1]["content"]
    schema = P.schema_block(fields, ob_fields)
    a = "%s\n\n" % P.RULEBOOK_TEXT
    b = a + "%s\n\n" % schema
...
def main():
    ...
    ap.add_argument("--doc", default="REG-0007")
    ...
    system, steps = prefixes(text, fields, ob_fields)
