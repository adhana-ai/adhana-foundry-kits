...
function drawComputed(reserve, flag) {
  $("c-reserve").textContent = (reserve === null || reserve === undefined)
    ? "not computed — a required input was missing" : ("$" + reserve.toFixed(2));
  $("c-flag").textContent = (flag === null || flag === undefined)
    ? "not computed" : (flag ? "YES — route for underwriter review" : "no");
}
...
async function go() {
  ...
  try {
    ...
    draw(r.fields);
    drawComputed(r.computed_reserve_value, r.large_deposit_flag);
