"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-label-restrict --doc LBL-0032 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE CHECK SET. The decision is a walk
of eight ordered restrictions, so the walk has to be in the call -- which makes "how much of every
call is the check set" a question a reader should be able to answer with a number rather than a
guess. It is also the part that would grow with a real label, which is why it is worth pricing on
its own rather than folded into "the prompt".
"""
import argparse
import json
import os
