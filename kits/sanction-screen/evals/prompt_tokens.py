"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-sanction-screen --doc SCR-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. "Which identifier
outranks which" is a policy, so the policy has to travel in the call -- which makes "how much of
every call is the rulebook" a question a reader should be able to answer with a number rather than
a guess. On this kit the answer is uncomfortable, and publishing it is the point: the alert itself
is a small minority of what is paid for on every call.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields):
    ...
    return system, [("", "chat protocol and system prompt"),
                    (a, "adjudication rulebook"),
                    (b, "field schema"),
                    (full_user, "alert sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="SCR-0001")
