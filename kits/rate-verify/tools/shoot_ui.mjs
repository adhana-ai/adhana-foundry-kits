...
try {
  ...
  // ⚠︎ 1040 HIGH, MATCHING THE SIBLING KIT DIRECTLY BEFORE THIS ONE. This kit's payoff is the
  // ROUTING panel under the field table -- ten field rows plus three routed rows -- and at 820
  // the 'Needs a billing follow-up' row falls below the fold.
  ...
  if (LIVE) {
    // UTL-0005 exercises every moving part at once: an Industrial account on an interval meter
    // at exactly 15,000 kWh -- the TOU-8-outranks-demand boundary -- with a peak demand of 48 kW
    // (which alone would look like a GS-1 case), applied_rate_code GS-1, an account note reading
    // "Routine account. No concerns from billing on this one." (the wrong register -- this
    // account IS misrated), and bill_status "sent", so the pure-code rule routes it.
    await page.select('#doc', 'UTL-0005')
    await sleep(300)
  }
