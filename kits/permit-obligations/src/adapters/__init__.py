...
class AdapterError(RuntimeError):
    ...
    a socket timeout, a DNS failure, a half-read body. See the TRANSPORT note below — this kit
    SHIPS that distinction from birth rather than paying for it, because a sibling kit in this
    series already did: it lost two documents of a paid run to a timeout and a connection reset.
    """
...
def _post(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise AdapterError("%s %s: %s" % (e.code, e.reason, e.read().decode("utf-8", "replace")),
                           status=e.code)
    # ⚑ A DROPPED CONNECTION IS NOT A FAILED DOCUMENT EITHER. This clause is in this kit from its
    # first commit, and it is here because a SIBLING kit in this series lost two documents of a
    # paid run without it — one to `<urlopen error [Errno 60] Operation timed out>` and one to
    # `[Errno 54] Connection reset by peer`. Neither reached the model.
    #
    # ⚠︎ AND ON A MONITOR THE CONSEQUENCE IS WORSE THAN A LOST SCORE. A dropped register is not a
    # register with a bad worklist, it is a SITE NOBODY LOOKED AT — indistinguishable, downstream,
    # from a site with nothing due. That is precisely the confident silence this kit's whole
    # `not_determinable` status exists to avoid, and it would arrive through the transport layer
    # rather than through the model.
    #
    # ⚠︎ WHY IT IS EASY TO GET WRONG. A retry loop that keys only on an HTTP STATUS never sees a
    # TLS handshake timeout: a transport failure raises a bare OSError out of urlopen, which is not
    # an AdapterError at all, so it flies straight past four perfectly good backoff attempts and
    # out to the harness, which records it as a failed document on a run where the model was right.
    # The sibling comment further down already argues that a BUSY PROVIDER is not a failed
    # document; a DROPPED CONNECTION is the same argument one layer lower, and it is easy to
    # implement only the top half of it.
    #
...
def openai_compatible(cfg, system, user, max_tokens, thinking=None):
    ...
    return {"text": choice["message"]["content"],
            ...
            # calling truncated replies "failures" before anything read this field; the API had
            # been answering half the question the whole time and nothing looked.
            "finish_reason": choice.get("finish_reason"),
            # ⚑ WHERE THE BUDGET WENT. `finish_reason` says the reply was cut off; this says what
            # ate it. On this kit's calibration run it turned "why does an eleven-field record over
            # five conditions run to 2,100 tokens?" from a hypothesis into a number: 60.6 pct of the
            # output was a reasoning pass nobody asked for. Absent on providers that do not report
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    # ⚑ A BUSY PROVIDER IS NOT A FAILED DOCUMENT — first added in this series on 2026-08-03,
    # mid-run, after a live 503 "Service is too busy" dropped a document from a paid run of 57.
    #
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # ⚑ `or exc.transport` IS THE HALF THAT IS EASY TO LEAVE OUT. A reset connection and a
            # socket timeout have no status to test, so `exc.status not in TRANSIENT` is True for
            # both and a loop without this clause re-raises on the first attempt. Two documents of
            # a sibling kit's paid run were lost to exactly that.
