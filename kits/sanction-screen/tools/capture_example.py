...
"""Capture one reply VERBATIM, for the LLM lens's `raw_response`. SPENDS ONE CALL.
    ...
    python3 tools/capture_example.py --doc SCR-0006

⚑ WHY A FRESH CALL RATHER THAN A REPLY FROM THE SCORED RUN. evals/run.py records only the FIRST
alert's raw text, because storing 50 full replies in every result file makes the run record
...
though it were a 51st scored alert.
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--doc", default="SCR-0006")
    ap.add_argument("--beside", default="r001-sanction-screen")
    ...
    out = {
        ...
        "note": ("A fresh single call replayed after the scored run, to capture one reply "
                 "verbatim. It is NOT a 51st scored alert and is not counted in any denominator."),
        "model": cfg.get("model"),
        ...
        "needs_escalation": r.get("needs_escalation"),
        "recomputed_verdict": r.get("recomputed_verdict"),
        "recomputed_reason": r.get("recomputed_reason"),
        "recomputed_deciding_identifier": r.get("recomputed_deciding_identifier"),
        "recomputed_would_settle_it": r.get("recomputed_would_settle_it"),
        "document_text": text,
    }
    ...
    print("\ninput %s  output %s  finish %s" % (out["input_tokens"], out["output_tokens"],
                                                out["finish_reason"]))
    print("needs_escalation %r   recomputed %r / %r"
          % (out["needs_escalation"], out["recomputed_verdict"],
             out["recomputed_deciding_identifier"]))
