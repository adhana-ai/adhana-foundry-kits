...
Usage:
    python -m evals.run --run-id r001-<model>            # the real run, one call per certificate
...
def stub_complete(cfg, system, user, max_tokens=1024):
    ...
    out = {}
    m = re.search(r"Batch\n-+\n(.+)", user)
    out["batch_id"] = m.group(1).strip() if m else None
    m = re.search(r"Product\n-+\n(.+)", user)
    out["product_name"] = m.group(1).strip() if m else None
...
def main():
    ...
    o = scored["overall"]
    sc = flag_scored["self_consistency"]
    ext_hit = sum(1 for c in scored["cells"] if c["verdict"] == "hit")
    print("\n%-30s %s" % ("run", a.run_id))
    print("%-30s %d/%d  (%s)" % ("extraction accuracy", ext_hit, o["extraction_cells"],
                                 o["extraction_accuracy"]))
    print("%-30s %s" % ("conformance accuracy", flag_scored["accuracy"]))
    print("%-30s %s" % ("out-of-spec recall", flag_scored["recall"]))
    print("%-30s %d" % ("out-of-spec missed", flag_scored["false_negative"]))
    print("%-30s %d fired, catching %d of %d verdict errors with no gold"
          % ("self-consistency guardrail", sc["needs_review_fired"],
             sc["errors_caught_by_needs_review"], sc["verdict_errors"]))
