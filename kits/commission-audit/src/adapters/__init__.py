...
class AdapterError(RuntimeError):
    ...
    `transient` says "ask again shortly" for a failure that has NO status code at all — a
    connection that never completed. See the note on `_post` below for why that state had to be
    spelled out separately rather than folded into `status is None`.
    """

    def __init__(self, msg, status=None, transient=False):
        super().__init__(msg)
        self.status = status
...
def _post(url, headers, payload, timeout=120):
    """⚑ A TRANSPORT FAILURE IS RETRIED TOO — added 2026-08-22, AFTER a paid run lost a document
    to one, not before.

    ⚠︎ THIS IS A REAL DEFECT THIS KIT PAID FOR, AND THE RUN IT COST IS PUBLISHED AS IT RAN. The
    deliberating-tier run of this kit (r002-commission-audit) lost CMA-0013 to
    `<urlopen error [Errno 60] Operation timed out>` — 54 of 55 documents scored instead of 55.
    The retry loop below it was already correct about HTTP status codes and completely blind to
    the case where there is no status code, because `urlopen` raises a bare `URLError` (or a
    `socket.timeout`) that `except urllib.error.HTTPError` never sees. The loop then asked
    `exc.status not in TRANSIENT` of an exception that was not an AdapterError at all, so it
    propagated on the first attempt and the harness recorded a lost document.

    A connection that never completed returned no completion and was billed for nothing, so asking
    again is finishing the first attempt rather than taking a second one — exactly the reasoning
    the 503 path already used. What was missing was the classification, not the policy.

    ⚠︎ IT IS NOT `status is None`. `None` is also what an unclassified programming error would
    carry, and defaulting the unknown to "retry" is how a bug becomes four bugs and a backoff.
    """
    ...
    try:
        ...
        # No status, no completion, nothing billed. HTTPError is a subclass of URLError and is
        # already handled above, so it cannot reach here; socket.timeout is TimeoutError on 3.10+.
        raise AdapterError("transport failure reaching the provider: %r" % (e,), transient=True)
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # ⚑ TWO WAYS TO BE TRANSIENT: a status code that says "busy", or no connection at all.
            # The second was missing until a paid run lost a document to it — see _post's note.
            if not (exc.status in TRANSIENT or exc.transient) or attempt == RETRIES:
                raise
            last = exc
