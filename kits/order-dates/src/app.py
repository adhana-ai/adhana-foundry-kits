...
ONE SCHEDULING ORDER, ONE EXTRACTION, on your machine, with your key. It is the proof that this
runs on a laptop; it is NOT the published dashboard, which lives on the Foundry site from committed
run records.

IT RENDERS WITH NO KEY. /api/fields, /api/doc and /api/rulebook need nothing; only /api/calendar
...
data/rulebook.json verbatim, so the holiday list and the counting rules a date was computed by are
one click away from the date itself -- and the "invented, not an authority" line the reader needs
is on the same screen as the answer rather than in a file they will never open.

⚑ AND BOTH ANSWERS ARE SHOWN SIDE BY SIDE. The model's own `due_date` and the pure-code
recomputation from the model's own values sit in adjacent columns, with a marker where they
disagree. A page that showed one of them would be hiding the only check a forker gets on an order
nobody has labelled.
"""
import argparse
import errno
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8854 was free when this kit was written;
# `lsof -nP -iTCP:8854 -sTCP:LISTEN` says whether it still is, and --port moves it either way.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not config.has_key(cfg):
            return self._send(200, {"id": sid, "deadlines": None,
        ...
        try:
            ...
            return self._send(200, {"id": sid, "deadlines": None, "note": msg[:500]})
...
def main():
    ...
    print("order-dates UI  ->  http://127.0.0.1:%d" % a.port)
    print("  orders: %d   fields: %d   API_KEY: %s"
          % (len(EX.documents()), len(EX.load_fields()),
             "set" if config.has_key(cfg) else "not set (the page still renders)"))
    print("  the shipped rulebook (%s) is INVENTED and is not an authority; this kit computes a "
          "proposed calendar and never files, serves, dockets or waives anything" % CR.R["code"])
    srv.serve_forever()
