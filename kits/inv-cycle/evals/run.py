...
Usage:
    ...
    python -m evals.run --run-id r001-inv-cycle                   # THE REAL RUN, one call/event
    python -m evals.run --run-id r001-x --limit 5                 # a costed toe in the water

⚠︎ ONE CALL PER EVENT, COUNTED BEFORE IT IS MADE -- same discipline as every sibling kit's run.py.
...
def stub_complete(cfg, system, user, max_tokens=1024, **kw):
    """A deterministic fake provider: always says 'unresolved' with no citations. Not a baseline
    -- evals/baseline.py is; this only exercises prompt -> call -> parse -> score end to end with
    no key."""
    body = {"cause": "unresolved", "citations": [],
           "narrative": "Stub narrative exercising the pipeline only."}
    return {"text": json.dumps(body), "input_tokens": len(user) // 4,
           "output_tokens": 25, "raw": {"stub": True}}
...
def main():
    ...
    for i, event in enumerate(events, 1):
        eid = event["event_id"]
        t0 = time.time()
        r = B.draft(cfg, event, complete=complete, thinking=thinking, prompt=prompt_v)
        ...
        if first is None and r["replied"]:
            first = r
        records.append({
            "event_id": eid, "packed": r["packed"], "pack_meta": r["pack_meta"],
            "answer": r["answer"], "replied": r["replied"], "finish_reason": r.get("finish_reason"),
            "input_tokens": r.get("input_tokens"), "output_tokens": r.get("output_tokens"),
            "reasoning_tokens": r.get("reasoning_tokens"), "latency_ms": ms,
            "raw_text": r.get("raw_text", "") if not r["replied"] else None,
        })
        print("  %2d/%d  %-10s cause=%s" % (i, len(events), eid, r["answer"].get("cause")))
    ...
    out = {
        ...
        "events": len(records), "no_answer_events": no_answer,
