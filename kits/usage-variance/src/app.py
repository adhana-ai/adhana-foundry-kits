...
ONE USAGE-TO-INVOICE RECONCILIATION RECORD, ONE EXTRACTION, on your machine, with your key. It is
the proof that this runs on a laptop; it is NOT the published dashboard, which lives on the Foundry
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8811 rather than the next number after the
# highest kit in the repo: this kit was built alongside eight siblings in one sitting, all of which
# were claiming ports out of the same run of numbers at the same time, and "the next free one" is
# only free until somebody else takes it. A gap above the block is cheaper than a collision.
...
def main():
    ...
    print("usage-variance UI  ->  http://127.0.0.1:%d" % a.port)
    print("  records: %d   fields: %d   API_KEY: %s"
