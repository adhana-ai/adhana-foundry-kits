...
"""Capture one reply VERBATIM, for the LLM lens's `raw_response`. SPENDS ONE CALL.
    ...
    python3 tools/capture_example.py --doc REG-0007

⚑ WHY A FRESH CALL RATHER THAN A REPLY FROM THE SCORED RUN. evals/run.py records only the FIRST
register's raw text, because storing 50 full replies in every result file makes the run record
...
though it were a 51st scored register.
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--beside", default="r001-permit-obligations")
    ...
    r = EX.extract(cfg, text, fields, ob_fields)

    secs = segment.sections(text)
    _msgs, parts, used = P.build(text, secs, fields, ob_fields, selector)
    ...
    out = {
        ...
        "register": {k: v["value"] for k, v in r["fields"].items()},
        "obligations": [{"condition_id": o["condition_id"],
                         "values": o["values"],
                         "spans": {k: (v["span"]["section"] if v["span"] else None)
                                   for k, v in o["cells"].items()},
                         "status": o["status"], "due_date": o["due_date"],
                         "days_to_due": o["days_to_due"], "reason": o["reason"],
                         "undetermined_because": o["undetermined_because"]}
    ...
    print("\ninput %s  output %s  finish %s" % (out["input_tokens"], out["output_tokens"],
                                                out["finish_reason"]))
    print("worklist %r   escalate %r" % (out["worklist"], out["escalate"]))
