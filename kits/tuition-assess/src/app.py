...
ONE STUDENT-ACCOUNT TUITION ASSESSMENT, ONE EXTRACTION, on your machine, with your key. It is the
proof that this runs on a laptop; it is NOT the published dashboard, which lives on the Foundry
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8841 rather than the next free number
# after the last kit: nine kits were being built in parallel when this one was written, all of them
# reaching for "the next free port" at the same time, so this one is keyed to the use case's own
# id (UC041) instead. A number derived from something unique cannot collide by being obvious.
PORT = int(os.environ.get("PORT", "8841"))
...
def main():
    ...
    print("tuition-assess UI  ->  http://127.0.0.1:%d" % a.port)
    print("  assessments: %d   fields: %d   API_KEY: %s"
