...
//
// Ported from gap-brief's copy, same shape: this kit's UI is also one session, one call, no
// multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/session need
// nothing, and clicking Draft review brief with no API_KEY configured returns a calm 200 with no
// call made.
//
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.sessions) && j.sessions.length === 40
            && j.sessions.every((d) => /^SESS-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as alloc-short is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#sid', 'SESS-002')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'alloc-short-landing.png'), fullPage: true })
  console.log('  wrote alloc-short-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'alloc-short-nokey.png'), fullPage: true })
    console.log('  wrote alloc-short-nokey.png')
  } else {
