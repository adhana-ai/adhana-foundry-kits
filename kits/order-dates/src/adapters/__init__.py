...
class AdapterError(RuntimeError):
    ...
    distinction was added here after it cost this kit two documents of a paid run.
    """
...
def _post(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise AdapterError("%s %s: %s" % (e.code, e.reason, e.read().decode("utf-8", "replace")),
                           status=e.code)
    # ⚑ A DROPPED CONNECTION IS NOT A FAILED DOCUMENT EITHER — SHIPPED IN THIS KIT FROM BIRTH,
    # because a sibling kit in this series paid for it: its first paid run of 55 lost TWO documents,
    # one to `<urlopen error [Errno 60] Operation timed out>` and one to `[Errno 54] Connection
    # reset by peer`, and had to be re-fired. On this kit the cost of the same defect would be
    # worse than one lost row: a dropped order is up to six obligations with no dates at all, and a
    # calendar missing six entries looks exactly like an order that set six fewer deadlines.
    #
...
def openai_compatible(cfg, system, user, max_tokens, thinking=None):
    ...
    # from. This kit's harness never sends it, and the run records say so with an explicit null
    # rather than by omission -- a run that does not record the field is indistinguishable from one
    # that sent it. Whether disabling provider-side reasoning changes this kit's arithmetic is
    # UNMEASURED and is named as unmeasured on the kit's own page; it is the knob that would test
    # it, and firing that test is the operator's call, not this file's.
    ...
    return {"text": choice["message"]["content"],
            ...
            # ⚑ THE PROVIDER SAYS WHY IT STOPPED — RECORD IT. A sibling kit in this series spent
            # three published runs calling truncated replies "failures" while the API had been
            # answering half the question in this field the whole time and nothing read it. On this
            # kit a truncated reply costs a whole ORDER — every obligation in it — so the harness
            # tests this value explicitly before it calls a document lost.
            "finish_reason": choice.get("finish_reason"),
            # ⚑ WHERE THE BUDGET WENT. `finish_reason` says the reply was cut off; this says what
            # ate it. On the sibling kit that added it first, it turned "why does a short JSON
            # record run to the ceiling?" from a hypothesis into a number in one run — a median of
            # 100 pct of that kit's ceiling was a reasoning pass nobody asked for. Absent on
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # and the loop re-raised on the first attempt. Two documents of a sibling kit's paid 55
            # were lost to exactly that before this clause existed; this kit ships with it.
