...
// reproduce when the UI changes. Ported from param-drift's copy, same shape: this kit's UI is
// also one cycle, one call, no multi-step flow to drive.
//
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.cycles) && j.cycles.length === 40
            && j.cycles.every((d) => /^GB-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as gap-brief is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#cid', 'GB-0001')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'gap-brief-landing.png'), fullPage: true })
  console.log('  wrote gap-brief-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'gap-brief-nokey.png'), fullPage: true })
    console.log('  wrote gap-brief-nokey.png')
  } else {
