...
function drawComputed(income, flag) {
  $("c-income").textContent = (income === null || income === undefined)
    ? "not computed — a required input was missing" : ("$" + income.toFixed(2));
...
async function go() {
  ...
  try {
    ...
    draw(r.fields);
    drawComputed(r.qualifying_monthly_income, r.needs_review);
