"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-obligation-extract --doc OBX-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. The two calls this kit
makes are lookups over a shipped rule, so the rule has to be in the call -- which makes "how much
of every call is the rulebook" a question a reader should be able to answer with a number rather
than a guess.
"""
import argparse
import json
...
def prefixes(doc_text, fields):
    ...
    schema = P.field_schema(fields["contract_fields"], fields["fields"])
    a = "%s\n\n" % P.RULEBOOK_TEXT
    b = a + "Return these:\n%s\n\n" % schema
    ...
    return system, [("", "chat protocol and system prompt"),
                    (a, "worksheet rulebook"),
                    (b, "field schema"),
                    (full_user, "contract pack sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="OBX-0001")
