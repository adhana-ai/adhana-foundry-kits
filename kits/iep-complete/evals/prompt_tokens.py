"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-iep-complete --doc IEP-0001 --yes
...
⚑ THIS KIT HAS FIVE PARTS, AND TWO OF THEM ARE THE RULEBOOK. A completeness check is a lookup
against somebody's list of required elements, so the list has to be in the call -- and so does the
definition of the four states it can be in. That makes "how much of every call is the rulebook"
a question a reader should be able to answer with a number rather than a guess.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields):
    ...
    a = "%s\n\n" % P.RULEBOOK_TEXT
    b = "%s%s\n\n" % (a, P.STATE_TEXT)
    c = b + "Extract these fields:\n%s\n\n" % schema
    assert full_user.startswith(c), "prompt shape changed — this measurement would be wrong"
    return system, [("", "chat protocol and system prompt"),
                    (a, "component rulebook"),
                    (b, "component states"),
                    (c, "field schema"),
                    (full_user, "plan sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="IEP-0001")
