...
// also one record (here, one receiving-discrepancy case), one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/case need
// nothing, and clicking Resolve with no API_KEY configured returns a calm 200 with no call made. A
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.cases) && j.cases.length === 36
            && j.cases.every((d) => /^CASE-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as rcv-disc is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  // 1. The landing state: the first shipped case loaded, its lines and carrier-exception panel
  //    populated, no call made yet. fullPage so every line and the exception note -- the whole
  //    point of this panel -- are not cropped below the fold.
  await page.screenshot({ path: path.join(OUT, 'rcv-disc-landing.png'), fullPage: true })
  console.log('  wrote rcv-disc-landing.png')

  // 2. Press "Resolve" with no API_KEY configured -- a real state, not a mock. Free-by-
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'rcv-disc-nokey.png'), fullPage: true })
    console.log('  wrote rcv-disc-nokey.png')
  } else {
