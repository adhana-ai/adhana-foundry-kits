...
Usage:
    python -m evals.run --run-id r001-<model>            # the real run, one call per report
    python -m evals.run --run-id t000 --stub             # no key, no spend, proves the wiring
    python -m evals.run --run-id b000-rules --baseline    # free, heading-only, no model
"""
import argparse
import json
import os
...
def stub_complete(cfg, system, user, max_tokens=1024):
    import re
    out = {}
    m = re.search(r"Subject Property\n-+\n(.+)", user)
    out["property_address"] = m.group(1).strip() if m else None
    m = re.search(r"Appraiser\n-+\n(.+)", user)
    out["appraiser_name"] = m.group(1).strip() if m else None
...
def main():
    ...
    print("%-22s %s" % ("EA-flag recall", flag_scored["recall"]))
