...
- Purchase   
 ...
 SQLite Foreign Key Support   Table Of Contents 
...
```
 ...
 This way, the constraint is enforced by SQLite. Attempting to insert a row into the track table that does not correspond to any row in the artist table will fail, as will attempting to delete a row from the artist table when there exist dependent rows in the track table. There is one exception: if the foreign key column for an entry in the track table is NULL, then no corresponding entry in the artist table is required. Expressed in SQL, this means that for every row in the track table, the following expression evaluates to true: 
...
sqlite> -- does not correspond to any row in the artist table.
...
sqlite> -- the track table contains a row that refers to it.
...
- 
The parent table is the table that the foreign key constraint refers to. The parent table in the example in this section is the artist table. Some books and articles refer to this as the referenced table, which is arguably more correct, but tends to lead to confusion. 
- 
The child table is the table that the foreign key constraint is applied to and the table that contains the REFERENCES clause. The example in this section uses the track table as the child table. Other books and articles refer to this as the referencing table. 
- 
...
- 
 Foreign keys may not cross schema boundaries. That is, in REFERENCES (X.Y) table X will only be resolved within the schema which contains the REFERENCES clause.  
This page was last updated on 2026-03-20 12:37:00Z
