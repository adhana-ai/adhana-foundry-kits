...
The cut is on the underlined headings build_corpus.py writes ("Register Date", or
"Condition C-7.3", over a run of dashes). That is a property of this corpus's own layout, so `sections()` falls back to a single
whole-document segment when it finds no headings at all rather than pretending: a forker's
registers will not have these headings, and one honest segment beats a spurious carve-up.

⚠︎ ON THIS CORPUS THE SECTIONS ARE ALSO THE ROWS. Every condition on a register is its own
headed block, so `sections()` is what makes a per-condition span possible at all -- see
src/select.py::for_condition(). A register that arrives unheaded still reads, and every value on it
ships with no span rather than with a span pointing at the whole document.
"""
import re
