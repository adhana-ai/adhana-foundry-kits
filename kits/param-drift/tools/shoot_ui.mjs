...
// also one parameter, one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state, /api/parameter and
// /api/floor need nothing, and clicking Check with no API_KEY configured returns a calm 200 with
// no call made.
//
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.parameters) && j.parameters.length === 60
            && j.parameters.every((d) => /^PD-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as param-drift is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#pid', 'PD-0018')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'param-drift-landing.png'), fullPage: true })
  console.log('  wrote param-drift-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'param-drift-nokey.png'), fullPage: true })
    console.log('  wrote param-drift-nokey.png')
  } else {
