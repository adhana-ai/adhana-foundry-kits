...
IT RENDERS WITH NO KEY. /api/doc and /api/rulebook need nothing; only /api/check calls a provider,
...
# docs-redline 8769, docs-verify 8770. Starting this kit while an earlier one is still up must not
# die with a nine-line traceback ending in "Address already in use" — see the handling below.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/api/state":
            rb = C.rulebook()
            return self._send(200, {"documents": C.documents(),
                                    "has_key": config.has_key(),
                                    "rule_count": rb["rule_count"],
                                    "rulebook_section": rb["section"],
                                    "rulebook_edition": rb["edition"]})
        if u.path == "/api/rulebook":
            return self._send(200, C.rulebook())
        if u.path == "/api/doc":
            doc_id = (parse_qs(u.query).get("id") or [""])[0]
            if doc_id not in C.documents():
                return self._send(404, {"error": "no such document"})
            return self._send(200, {"id": doc_id, "text": C.load_doc(doc_id)})
    ...
    def do_POST(self):
        ...
        # both are present, so a "paste a document" box needs no server-side change to work — this
        # endpoint already accepts arbitrary text, it just also knows how to look one up by id.
        doc_id = req.get("id")
        text = req.get("text")
        if text is None:
            if doc_id not in C.documents():
                return self._send(404, {"error": "no such document, and no text was supplied"})
            text = C.load_doc(doc_id)
        cfg = config.load()
        if not config.has_key(cfg):
            # 200, not an error: "no key" is a configuration state the page should render calmly.
            return self._send(200, {"id": doc_id, "rules": None,
                                    "note": "No API_KEY is configured, so nothing was called. "
                                            "Copy .env.example to .env and set one."})
        try:
            # ⚠︎ THINKING IS EXPLICITLY DISABLED HERE, NOT LEFT AT THE PROVIDER DEFAULT. UC006 lost
            # a run to a provider that reasons by default and burned the whole output ceiling on
            # hidden reasoning, returning nothing; docs-verify then fixed that in evals/run.py and
            # left its own UI reproducing the defect on every click. This kit's ceiling is the
            # largest in the estate (41 rules in one reply), so it has the most to lose.
            r = C.check(cfg, text, C.rules(), thinking=adapters.THINKING_OFF)
            ...
            return self._send(200, {"id": doc_id, "rules": None, "note": msg[:500]})
        return self._send(200, {
            "id": doc_id,
            "rules": r["rules"],
            "summary": C.summary(r),
            "answered": r["answered"], "asked": r["asked"],
            # `parsed` is carried through rather than inferred from an empty list — the third
            # state, same as everywhere else in this kit: "answered nothing" and "could not be
            # read" are different facts and the page is entitled to tell them apart.
            "parsed": r["parsed"],
...
def main():
    ...
    rb = C.rulebook()
    print("docs-comply UI  ->  http://127.0.0.1:%d" % a.port)
    print("  documents: %d   rules: %d (%s, %s edition)   API_KEY: %s"
          % (len(C.documents()), rb["rule_count"], rb["section"], rb["edition"],
