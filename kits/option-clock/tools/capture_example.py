...
"""Capture one reply VERBATIM, for the LLM lens's `raw_response`. SPENDS ONE CALL.
    ...
    python3 tools/capture_example.py --doc ROR-0033
...
def main():
    ...
    ap.add_argument("--doc", default="ROR-0033")
    ap.add_argument("--beside", default="r001-option-clock")
    ...
    out = {
        ...
        "escalate_now": r.get("escalate_now"),
        "counted_status": r.get("counted_status"),
        "counted_expiry_date": r.get("counted_expiry_date"),
        "counted_clock_start_date": r.get("counted_clock_start_date"),
        "counted_days_to_expiry": r.get("counted_days_to_expiry"),
        "counted_reason": r.get("counted_reason"),
        "document_text": text,
    }
    ...
    print("\ninput %s  output %s  finish %s" % (out["input_tokens"], out["output_tokens"],
                                                out["finish_reason"]))
    print("escalate_now %r   counted %r on %r"
          % (out["escalate_now"], out["counted_status"], out["counted_expiry_date"]))
