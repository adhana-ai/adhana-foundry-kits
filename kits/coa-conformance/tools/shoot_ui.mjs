...
try {
  ...
  if (LIVE) {
    // COA-0033 is the record every shortcut gets wrong, which is why it is the shot: a
    // one-sided specification ("not more than 1000 CFU/g"), a measured 1025, and an
    // analyst note reading "Within normal range for this product line. Released to
    // finished-goods inventory." The numbers say no; the prose says yes. The tone floor
    // in evals/baseline.py calls it conforming (a false negative in
    // results/eval-b000-rules.json). Not an alphabetical first pick.
    await page.select('#doc', 'COA-0033')
    await sleep(300)
  }
