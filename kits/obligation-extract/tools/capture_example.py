...
"""Capture one reply VERBATIM, for the LLM lens's `raw_response`. SPENDS ONE CALL.
    ...
    python3 tools/capture_example.py --doc OBX-0004

⚑ WHY A FRESH CALL RATHER THAN A REPLY FROM THE SCORED RUN. evals/run.py records only the FIRST
pack's raw text, because storing 50 full list-shaped replies in every result file makes the run
...
as though it were a 51st scored pack.
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--doc", default="OBX-0004")
    ap.add_argument("--beside", default="r001-obligation-extract")
    ...
    msgs, parts, used = P.build(text, secs, fields, selector)

    rows = []
    for row in r["obligations"]:
        rows.append({n: c["value"] for n, c in row.items() if n != "_recomputed"})

    out = {
        "doc": a.doc,
        "beside_run": a.beside,
        "note": ("A fresh single call replayed after the scored run, to capture one reply "
                 "verbatim. It is NOT a 51st scored pack and is not counted in any denominator."),
        "model": cfg.get("model"),
        ...
        "prompt_verbatim": msgs[0]["content"] + "\n\n" + msgs[1]["content"],
        "contract": {k: v["value"] for k, v in r["contract"].items()},
        "obligations": rows,
        "spans": [{n: (c["span"]["section"] if c.get("span") else None)
                   for n, c in row.items() if n != "_recomputed"} for row in r["obligations"]],
        "needs_drafting_review": r.get("needs_drafting_review"),
        "document_text": text,
    }
    ...
    print("\ninput %s  output %s  finish %s" % (out["input_tokens"], out["output_tokens"],
                                                out["finish_reason"]))
    print("needs_drafting_review %r   lines %d" % (out["needs_drafting_review"], len(rows)))
