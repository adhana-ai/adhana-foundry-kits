...
ONE WARRANTY CLAIM RECORD, ONE ADJUDICATION, on your machine, with your key. It is the proof that
this runs on a laptop; it is NOT the published dashboard, which lives on the Foundry site from
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. This one is derived from the use case's
# own permanent id (UC046 -> 8946) rather than from "the next free number after the last kit",
# which is how two kits written in parallel end up on the same port and neither author notices.
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/api/doc":
            ...
            if sid not in EX.documents():
                return self._send(404, {"error": "no such claim"})
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                ...
                                "recomputed_covered": r["recomputed_covered"],
                                "recomputed_months": r["recomputed_months"],
...
def main():
    ...
    print("coverage-check UI  ->  http://127.0.0.1:%d" % a.port)
    print("  claims: %d   fields: %d   API_KEY: %s"
