...
// reproduce when the UI changes. Ported from docs-comply's copy, same shape: this kit's UI is
// also one order, one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/order need
// nothing, and clicking Reconcile with no API_KEY configured returns a calm 200 with no call
// made. A third, --live shot is deliberately NOT taken by this pass: r001-data-reconcile and
// r002-data-reconcile-pro are already on disk with committed numbers, and a screenshot is not
// where a measurement should come from.
//
...
// ⚠︎ REFUSE TO RUN IF THE PORT IS ALREADY TAKEN -- same discipline docs-comply's script paid a
// real provider call to learn. This script blanks API_KEY so its shots are free; a server it did
// not start may hold a real key, and clicking would spend on it.
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.orders) && j.orders.length === 72
            && j.orders.every((d) => /^ORD-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as data-reconcile is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  // 1. The landing state: the first shipped order loaded, the agreement panel populated, no
  //    call made yet. fullPage so the stats grid and the reconciliation line -- the whole point
  //    of this panel -- are not cropped below the fold.
  await page.screenshot({ path: path.join(OUT, 'reconcile-landing.png'), fullPage: true })
  console.log('  wrote reconcile-landing.png')

  // 2. Press "Reconcile" with no API_KEY configured -- a real state, not a mock. Free-by-
  //    construction: the server above is started with the key blanked in its own environment,
  //    so this cannot become the mistake this kit's own build session made once already
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'reconcile-nokey.png'), fullPage: true })
    console.log('  wrote reconcile-nokey.png')
  } else {
