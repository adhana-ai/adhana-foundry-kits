...
The cut is on the underlined headings build_corpus.py writes ("Extension One" over a run of
...
⚠︎ ON THIS CORPUS THE SECTION BOUNDARY IS ALSO WHAT KEEPS TWO DATES APART. A rights register
states a grant date, a triggering-event date and a payment date within a few lines of each other,
in the same ISO format. An unscoped search for "2026-04-11" would happily cite the payment line
for a trigger date that is genuinely correct, so src/extract.py::_locate searches the sections
src/select.py maps a field to BEFORE falling back to the whole document.
"""
import re
