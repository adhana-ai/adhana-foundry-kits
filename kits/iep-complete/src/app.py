...
ONE EDUCATION PLAN, ONE CHECK, on your machine, with your key. It is the proof that this runs on a
laptop; it is NOT the published dashboard, which lives on the Foundry site from committed run
records.
...
verbatim, so the list of required elements the answer was decided by is one click away from the
answer itself -- and the "invented, not an authority" line the reader needs is on the same screen as
the worklist rather than in a file they will never open.
"""
import argparse
import errno
import json
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8957 was free when this kit was written;
# `lsof -nP -iTCP:8957 -sTCP:LISTEN` says whether it still is, and --port moves it either way.
PORT = int(os.environ.get("PORT", "8957"))
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/api/fields":
            return self._send(200, {"fields": EX.load_fields(),
                                    ...
                                    "states": list(P.VERDICTS),
                                    "state_meanings": P.VERDICT_MEANINGS,
                                    "outcome_meanings": P.OUTCOME_MEANINGS,
        ...
        if u.path == "/api/doc":
            ...
            if sid not in EX.documents():
                return self._send(404, {"error": "no such plan"})
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                "on_worklist": r["on_worklist"],
                                "reason": r["recomputed_reason"],
                                "missing": r["recomputed_missing"],
                                "unmeasurable": r["recomputed_unmeasurable"],
                                "recomputed_outcome": r["recomputed_outcome"],
...
def main():
    ...
    print("iep-complete UI  ->  http://127.0.0.1:%d" % a.port)
    print("  plans: %d   fields: %d   API_KEY: %s"
          % (len(EX.documents()), len(EX.load_fields()),
             "set" if config.has_key(cfg) else "not set (the page still renders)"))
    print("  the shipped rulebook is INVENTED and is not an authority; this kit produces a "
          "reviewer's worklist and never approves, signs, files or amends a plan")
    print("  every plan in data/corpus is generated -- no real pupil, school or district appears "
