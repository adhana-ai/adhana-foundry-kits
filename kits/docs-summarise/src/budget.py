...
⚑ A `HARD_CEILING = 300` CONSTANT LIVED HERE FOR A FEW HOURS ON 2026-08-07 AND WAS REMOVED THE SAME
EVENING, ON THE OPERATOR'S OWN EXPLICIT, REPEATED INSTRUCTION. It clamped this file's cap
regardless of `.env`, specifically so no chat exchange could raise it — built after the operator
watched the day's cap get raised three times with no real limit. It held through three separate
asks that evening to bypass it. Removed on a fourth, unambiguous instruction — "remove hard
ceiling everywhere... I don't want to put any hard ceiling at this moment" — which read as a
stated policy change rather than a workaround for one blocked run. `.env`'s `MAX_CALLS_PER_DAY` is,
once again, the only lever, and raising it is the operator's call, same as before this constant
ever existed.
"""
import json
import os
