...
// reproduce when the UI changes. Ported from data-reconcile's copy, same shape: this kit's UI is
// also one close cycle, one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/cycle need
// nothing, and clicking Check with no API_KEY configured returns a calm 200 with no call made. A
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.cycles) && j.cycles.length === 60
            && j.cycles.every((d) => /^CLS-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as fin-close is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  // 1. The landing state: the first shipped close cycle loaded, the basis panel populated, no
  ...
  await page.screenshot({ path: path.join(OUT, 'fin-close-landing.png'), fullPage: true })
  console.log('  wrote fin-close-landing.png')

  // 2. Press "Check" with no API_KEY configured -- a real state, not a mock. Free-by-
  //    construction: the server above is started with the key blanked in its own environment,
  //    so this cannot become the mistake data-reconcile's own build session made once already
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'fin-close-nokey.png'), fullPage: true })
    console.log('  wrote fin-close-nokey.png')
  } else {
