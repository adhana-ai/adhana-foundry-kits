...
// reproduce when the UI changes. Ported from fin-invval's copy, same shape: this kit's UI is
// also one record (here, one invoice's 4-stage trace), one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/invoice need
// nothing, and clicking Trace with no API_KEY configured returns a calm 200 with no call made. A
// third, --live shot is deliberately NOT taken by this pass: a real committed run's numbers should
// come from evals/run.py, not from a screenshot.
//
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.invoices) && j.invoices.length === 35
            && j.invoices.every((d) => /^INV-\d+-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as fin-payrun is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  // 1. The landing state: the first shipped invoice loaded, the 4-stage record panel populated,
  //    no call made yet. fullPage so the stage rows and the vendor inquiry -- the whole point of
  //    this panel -- are not cropped below the fold.
  await page.screenshot({ path: path.join(OUT, 'fin-payrun-landing.png'), fullPage: true })
  console.log('  wrote fin-payrun-landing.png')

  // 2. Press "Trace" with no API_KEY configured -- a real state, not a mock. Free-by-construction:
  //    the server above is started with the key blanked in its own environment, so this cannot
  //    become the mistake data-reconcile's own build session made once already (curl'ing /api/check
  //    by hand against a server that inherited a real shared key).
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'fin-payrun-nokey.png'), fullPage: true })
    console.log('  wrote fin-payrun-nokey.png')
  } else {
