...
docs-comply's FALSE MET. A verdict of `clean` where gold says `defect` is the expensive direction:
it ships a real violation with a tick on it. `false_alarm` (predicted `defect`, gold `clean`) costs
a buyer's trust in the queue but never lets a bad order through. The two are not interchangeable
and a single accuracy number hides which one a given error rate is made of.
