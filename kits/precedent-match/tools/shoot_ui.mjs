...
async function proveItIsOurs() {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.requests) && j.requests.length === 15
            && j.requests.every((d) => /^REQ-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as precedent-match is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#req', 'REQ-00002')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'precedent-match-landing.png'), fullPage: true })
  console.log('  wrote precedent-match-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'precedent-match-nokey.png'), fullPage: true })
    console.log('  wrote precedent-match-nokey.png')
  } else {
