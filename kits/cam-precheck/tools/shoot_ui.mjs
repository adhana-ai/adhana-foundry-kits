...
try {
  ...
  // ⚠︎ 1620 HIGH, THE TALLEST IN THIS SERIES, AND THE HEIGHT IS FORCED BY THE FIELD COUNT. This
  // kit extracts TWENTY fields where its siblings extract nine or ten, and its payoff is the
  // ROUTING panel BELOW all of them -- six rows ending in 'Needs a corrected statement' and the
  // re-run of the four stages on the reply's own numbers. At 1040 the entire panel is off-screen
  // and the shot shows a field table with no point to it.
  await page.setViewport({ width: 1280, height: 1620, deviceScaleFactor: 2 })
  ...
  if (LIVE) {
    // CAM-0004 exercises every moving part at once: an occupancy-sensitive line in a building at
    // 86 pct occupancy, so the gross-up fires; an annual cap whose ceiling has SLACK against the
    // ungrossed share and BINDS against the grossed-up one, so skipping stage 2 also silently
    // skips stage 4; a statement already issued; and an accountant note reading "Escalated for
    // manager review; the allocation basis on this line is disputed." -- the wrong register, since
    // this line is billed to the cent. The right answer is that nothing needs doing.
    await page.select('#doc', 'CAM-0004')
    await sleep(300)
  }
  ...
  if (btn) {
    ...
    if (LIVE) {
      await page.waitForFunction(
        () => !document.querySelector('#rows').textContent.includes('not pre-checked yet'),
        { timeout: 120000 })
      await sleep(400)
    } else {
      await sleep(1200)
    ...
    console.log('  !! no Pre-check control found — the UI changed; fix the selector above')
    process.exitCode = 1
  }
} finally {
