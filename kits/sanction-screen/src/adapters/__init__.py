...
def _post(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise AdapterError("%s %s: %s" % (e.code, e.reason, e.read().decode("utf-8", "replace")),
                           status=e.code)
    # ⚑ A DROPPED CONNECTION IS NOT A FAILED DOCUMENT EITHER — added here after this kit's first
    # paid run of 55 lost TWO sheets, one to `<urlopen error [Errno 60] Operation timed out>` and
    # one to `[Errno 54] Connection reset by peer`.
    #
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # and the loop re-raised on the first attempt. Two sheets of a paid 55 were lost to
            # exactly that before this clause existed.
