...
scored by the identical function (evals/scoring.py) -- the gap measured between them is entirely
in which pairs get counted as like items, not in who saw a smaller haystack or a friendlier
grader.
"""
import argparse
import json
...
def main():
    ...
    for req in requests:
        ...
        for c in cand:
            sim = SIM.score(req, c)
            merged = D.counts_as_like_item(sim)               # no verdict -- the pure floor
            per_candidate.append({"item_id": c["item_id"], "verdict": None, "reason": None,
