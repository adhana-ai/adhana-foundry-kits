"""Run the option clock over the corpus and write one result file. THIS SPENDS MONEY.

Usage:
    python -m evals.run --run-id r001-option-clock --yes             # the real run, one call each
    python -m evals.run --run-id t000-option-clock-stub --stub       # no key, no spend
    python -m evals.run --run-id b000-option-clock-register --baseline
    python -m evals.run --run-id c000-option-clock-calibration --limit 6 --max-tokens 16000 --yes
...
def load_gold():
    with open(GOLD, encoding="utf-8") as f:
        return {r["register_ref"]: r for r in (json.loads(l) for l in f if l.strip())}
...
def stub_complete(cfg, system, user, max_tokens=1024):
    ...
    segment, select, prompt, parse, span, the pure-code count, the escalation flag, the scorer --
    is exercised for free before anything is paid for."""
    import re
    out = {}
    m = re.search(r"Register\n-+\n(.+)", user)
    out["register_id"] = m.group(1).strip() if m else None
    m = re.search(r"Register As Of\n-+\n(.+)", user)
    out["register_as_of"] = m.group(1).strip() if m else None
...
def main():
    ...
    # call) and stay sequential (workers=1); a live run is 50 real HTTP calls and was the long pole
    # in this kit's build. `EX.extract` is a pure function with no shared mutable state, and
    # `complete()` in src/adapters already retries transient failures -- 429/5xx AND transport-level
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        for i, reg_id, r, err, dt in pool.map(process_one, items):
            ...
            records[reg_id] = r["fields"]
            published[reg_id] = {"status": r.get("published_status"),
                                 "expiry_date": r.get("published_expiry_date")}
            flags[reg_id] = r.get("escalate_now")
            if first is None:
                first = r
            print("  %3d/%-3d %-10s %d ms" % (i, len(docs), reg_id, lat[-1] if lat else 0))

    golds = load_gold()
    golds = {k: v for k, v in golds.items() if k in set(docs)}
    scored = J.score(fields, records, golds)
    clock = J.score_clock(records, published, flags, golds)

    lat_in_order = list(lat)
    lat.sort()
    p = lambda q: lat[min(len(lat) - 1, int(len(lat) * q))] if lat else None

    # ⚑ EVERY PUBLISHED FIGURE IS LIFTED INTO THIS ONE BLOCK ON PURPOSE. `false_alarm_rate_pct` and
    # `missed_lapse_count` are this kit's headline pair and they live two levels down under
    # `clock_scores`; a board extractor that has to walk two different depths to build one record is
    # an extractor that quietly drops the deeper half. They are copied, not moved -- `clock_scores`
    # keeps the full matrices and every named row.
    ef = clock["escalate_flag"]
    wl = clock["worklist"]
    overall = dict(
        scored["overall"],
        status_accuracy_pct=round(100.0 * clock["status_accuracy"], 2),
        status_correct=clock["status_correct"],
        status_rows=clock["status_rows"],
        # ⚑ THE NUMBER THAT MATTERS MOST IN A MONITOR KIT, AND IT IS FIRST FOR THAT REASON.
        false_alarm=clock["false_alarm_count"],
        false_alarm_rate_pct=clock["false_alarm_rate_pct"],
        # ⚑ AND THE EXPENSIVE ONE. An option wrongly reported live is one nobody opens.
        missed_lapse=clock["missed_lapse_count"],
        missed_lapse_rate_pct=clock["missed_lapse_rate_pct"],
        ...
        date_accuracy_pct=clock["dates"]["accuracy_pct"],
        date_rows=clock["dates"]["scored"],
        phantom_expiry=clock["dates"]["phantom_expiry_count"],
        escalate_flag_recall_pct=(None if ef["recall"] is None
                                  else round(100.0 * ef["recall"], 2)),
        escalate_flag_precision_pct=(None if ef["precision"] is None
                                     else round(100.0 * ef["precision"], 2)),
        claimed_status_accuracy_pct=clock["claimed_status"]["accuracy_pct"],
        note_contradiction_accuracy_pct=clock["note_contradiction"]["accuracy_pct"],
        self_consistency_disagreements=clock["consistency"]["replies_disagreeing_with_own_values"],
    )
    ...
    print("%-38s %d/%d  (%s)" % ("status accuracy, four-way",
                                 clock["status_correct"], clock["status_rows"],
                                 clock["status_accuracy"]))
    print("%-38s %d  (%s%% of the %d that are genuinely live)"
          % ("FALSE ALARMS", clock["false_alarm_count"], clock["false_alarm_rate_pct"],
             clock["false_alarm_denominator"]))
    print("%-38s %d  (%s%% of the %d that need somebody)"
          % ("MISSED LAPSES (reported live)", clock["missed_lapse_count"],
             clock["missed_lapse_rate_pct"], clock["missed_lapse_denominator"]))
    print("%-38s acc %s  recall %s  precision %s"
          % ("worklist (needs somebody vs not)", wl["accuracy"], wl["recall"], wl["precision"]))
    print("%-38s %d/%d  (%s%%)   phantom expiries: %d"
          % ("date arithmetic, counted expiry", dates["correct"], dates["scored"],
             dates["accuracy_pct"], dates["phantom_expiry_count"]))
    print("%-38s acc %s  recall %s  precision %s"
          % ("escalate_now vs gold", ef["accuracy"], ef["recall"], ef["precision"]))
    print("%-38s %s%% (%d of %d)"
          % ("the model's own stated status",
             clock["claimed_status"]["accuracy_pct"], clock["claimed_status"]["correct"],
             clock["claimed_status"]["rows"]))
    print("%-38s %s%% (%d of %d)"
          % ("...on registers the note contradicts",
             clock["note_contradiction"]["accuracy_pct"], clock["note_contradiction"]["correct"],
             clock["note_contradiction"]["rows"]))
    print("%-38s %d reply(s) disagreed with their own values; %d of %d status errors were "
          "visible without gold"
          % ("consistency diagnostic", cons["replies_disagreeing_with_own_values"],
             cons["errors_visible_without_gold"], cons["status_errors"]))
    if out_max:
