...
def _post(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise AdapterError("%s %s: %s" % (e.code, e.reason, e.read().decode("utf-8", "replace")),
                           status=e.code)
    # ⚑ A DROPPED CONNECTION IS NOT A FAILED DOCUMENT EITHER — AND THIS KIT SHIPS THE CLAUSE FROM
    # BIRTH RATHER THAN PAYING FOR IT. A sibling kit in this series lost TWO documents of a paid
    # run of 55 to exactly this hole, one to `<urlopen error [Errno 60] Operation timed out>` and
    # one to `[Errno 54] Connection reset by peer`; both were recorded as failed documents on a run
    # where the model was never asked. On a register check the consequence is specific and bad: a
    # dropped register produces no worklist, and an engagement with no worklist looks exactly like
    # an engagement nobody had to chase.
    #
...
def openai_compatible(cfg, system, user, max_tokens, thinking=None):
    ...
    # from. This kit never sends it: every published figure here was measured with the provider's
    # own default, which on this model family is reasoning ON. The parameter exists so a forker can
    # test what turning it off costs in accuracy; nothing in this kit has measured that, and the
    # kit page says so rather than implying the question was settled.
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    # ⚑ A BUSY PROVIDER IS NOT A FAILED DOCUMENT — the argument this repo first made on
    # 2026-08-03, mid-run, after a live 503 "Service is too busy" dropped a document from a paid
    # run of 57.
    #
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # ⚑ `or exc.transport` IS THE HALF THAT IS EASY TO LEAVE OUT. A reset connection and
            # a socket timeout have no status to test, so `exc.status not in TRANSIENT` is True for
            # both and a status-only loop re-raises on the first attempt. Two documents of a
            # sibling kit's paid run of 55 were lost to exactly that before this clause existed
            # there; it is here from the first commit.
