...
// also one message, one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state, /api/message and
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.messages) && j.messages.length === 70
            && j.messages.every((d) => /^MSG-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as change-impact is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#msg', 'MSG-00002')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'change-impact-landing.png'), fullPage: true })
  console.log('  wrote change-impact-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'change-impact-nokey.png'), fullPage: true })
    console.log('  wrote change-impact-nokey.png')
  } else {
