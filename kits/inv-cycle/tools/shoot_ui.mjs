...
// reproduce when the UI changes. Ported from mp-onboard's copy, same shape: this kit's UI is
// also one record (here, one variance event's own transaction history log), one call, no
// multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/event need
// nothing, and clicking "Draft cause" with no API_KEY configured returns a calm 200 with no call
...
// ⚠︎ REFUSE TO RUN IF THE PORT IS ALREADY TAKEN -- same discipline mp-onboard's own script states.
// This script blanks API_KEY so its shots are free; a server it did not start may hold a real
// key, and clicking would spend on it.
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.events) && j.events.length === 36
            && j.events.every((d) => /^VC-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as inv-cycle is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  // 1. The landing state: the first shipped event loaded, the variance and its transaction
  //    history log both populated, no call made yet. fullPage so every log line -- the whole
  //    point of this panel -- is not cropped below the fold.
  await page.screenshot({ path: path.join(OUT, 'inv-cycle-landing.png'), fullPage: true })
  console.log('  wrote inv-cycle-landing.png')

  // 2. Press "Draft cause" with no API_KEY configured -- a real state, not a mock. Free-by-
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'inv-cycle-nokey.png'), fullPage: true })
    console.log('  wrote inv-cycle-nokey.png')
  } else {
