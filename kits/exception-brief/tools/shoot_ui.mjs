...
// reproduce when the UI changes. Ported from gap-brief's copy, same shape: this kit's UI is also
// one batch, one call, no multi-step flow to drive.
//
// ⚠︎ IT NEVER SPENDS ON ITS OWN. Both shots here are free -- /api/state and /api/batch need
// nothing, and clicking Draft brief with no API_KEY configured returns a calm 200 with no call
// made.
//
...
async function proveItIsOurs() {
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (!r.ok) {
        ...
        if (Array.isArray(j.batches) && j.batches.length === 40
            && j.batches.every((d) => /^EB-\d+$/.test(d))) return j
  ...
  console.error(`  !! nothing recognisable as exception-brief is serving 127.0.0.1:${PORT}.`)
...
try {
  ...
  await page.select('#bid', 'EB-0001')
  await sleep(400)
  await page.screenshot({ path: path.join(OUT, 'exception-brief-landing.png'), fullPage: true })
  console.log('  wrote exception-brief-landing.png')
  ...
  if (btn) {
    ...
    await page.waitForFunction(
      ...
      { timeout: 15000 })
    await sleep(300)
    await page.screenshot({ path: path.join(OUT, 'exception-brief-nokey.png'), fullPage: true })
    console.log('  wrote exception-brief-nokey.png')
  } else {
