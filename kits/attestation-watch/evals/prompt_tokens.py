"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-attestation-watch --doc ATT-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. The cycle lengths and
the grace window are a lookup, so the table has to be in the call -- which makes "how much of every
call is the rulebook" a question a reader should be able to answer with a number rather than a
guess. And the answer here is not the one a reader expects: on this kit the FIELD SCHEMA is the
expensive part, because eleven per-person fields each need a hint long enough to be unambiguous.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields):
    ...
    msgs, _parts, _ = P.build(doc_text, secs, fields, selector)
    system, full_user = msgs[0]["content"], msgs[1]["content"]
    a = "%s\n\n" % P.RULEBOOK_TEXT
    b = full_user[:full_user.index("ATTESTATION REGISTER\n")]
    assert full_user.startswith(a), "prompt shape changed — this measurement would be wrong"
    assert b.startswith(a), "prompt shape changed — this measurement would be wrong"
    return system, [("", "chat protocol and system prompt"),
                    (a, "attestation rulebook"),
                    (b, "field schema and reply shape"),
                    (full_user, "register sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="ATT-0001")
