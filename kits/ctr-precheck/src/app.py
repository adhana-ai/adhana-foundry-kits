...
ONE QC PACK, ONE EXTRACTION, on your machine, with your key. It is the proof that this runs on a
...
data/rulebook.json verbatim, so the threshold, the window and the code table the answer was decided
by are one click away from the answer itself -- and the "invented, not an authority" line the reader
needs is on the same screen as the answer rather than in a file they will never open.
"""
import argparse
import errno
import json
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. 8856 was free when this kit was written;
# `lsof -nP -iTCP:8856 -sTCP:LISTEN` says whether it still is, and --port moves it either way.
PORT = int(os.environ.get("PORT", "8856"))
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        return self._send(200, {"id": sid, "fields": r["fields"],
                                "needs_recompute": r["needs_recompute"],
                                "recomputed_defects": r["recomputed_defects"],
                                "recomputed_reasons": r["recomputed_reasons"],
...
def main():
    ...
    print("ctr-precheck UI  ->  http://127.0.0.1:%d" % a.port)
    print("  packs: %d   fields: %d   API_KEY: %s"
          % (len(EX.documents()), len(EX.load_fields()),
             "set" if config.has_key(cfg) else "not set (the page still renders)"))
    print("  the shipped rulebook is INVENTED and is not an authority; this kit pre-checks a draft "
          "and files nothing and clears nothing")
    srv.serve_forever()
