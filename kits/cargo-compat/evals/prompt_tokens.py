"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-cargo-compat --doc CGO-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE MATRIX. A compatibility decision is
a lookup, so the table has to be in the call -- which makes "how much of every call is the matrix"
...
def prefixes(doc_text, fields):
    ...
    return system, [("", "chat protocol and system prompt"),
                    ...
                    (b, "field schema"),
                    (full_user, "check sheet sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="CGO-0001")
