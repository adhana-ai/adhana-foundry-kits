...
ONE ATTESTATION REGISTER, ONE CHECK, on your machine, with your key. It is the proof that this runs
on a laptop; it is NOT the published dashboard, which lives on the Foundry site from committed run
records.

⚠︎ AND IT IS NOT A MONITOR. There is no schedule, no poller, no subscription and no state. You pick
a register, it makes one call, it prints a proposed worklist, and then it does nothing at all until
you click again. Nothing here chases, files, signs a register off or clears anybody.

IT RENDERS WITH NO KEY. /api/fields, /api/doc and /api/rulebook need nothing; only /api/check calls
...
verbatim, so the table the status was decided by is one click away from the status itself -- and the
"invented, not an authority" line the reader needs is on the same screen as the answer rather than
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not config.has_key(cfg):
            return self._send(200, {"id": sid, "result": None,
                                    "note": "No API_KEY is configured, so nothing was called. "
                                            "Copy .env.example to .env and set one, or set it "
        ...
        try:
            ...
            return self._send(200, {"id": sid, "result": None, "note": msg[:500]})
        return self._send(200, {"id": sid, "result": {
            "register": r["register"],
            "attesters": [{k: a[k] for k in ("person_ref", "fields", "computed_status",
                                             "computed_reason", "computed_due_on",
                                             "computed_stale_after", "not_determinable_because")}
                          for a in r["attesters"]],
            "worklist": r["worklist"],
            "not_determinable": r["not_determinable"],
            "needs_owner_review": r["needs_owner_review"],
...
def main():
    ...
    f = EX.load_fields()
    print("attestation-watch UI  ->  http://127.0.0.1:%d" % a.port)
    print("  registers: %d   fields: %d register-level + %d per-person   API_KEY: %s"
          % (len(EX.documents()), len(f["register"]), len(f["attester"]),
             "set" if config.has_key(cfg) else "not set (the page still renders)"))
    print("  this kit WATCHES NOTHING -- it reads one snapshot and proposes a worklist; the "
          "shipped rulebook is INVENTED and is not an authority")
    srv.serve_forever()
