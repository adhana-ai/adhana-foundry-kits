...
try {
  ...
  if (LIVE) {
    // IV-0001: a generic "Deposit" line over threshold with no named counterparty -- the
    // statement that should trigger the review flag, not an arbitrary alphabetical first pick.
    await page.select('#doc', 'IV-0001')
    await sleep(300)
  }
