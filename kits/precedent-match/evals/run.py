...
def stub_complete(cfg, system, user, max_tokens=1024, **kw):
    """A deterministic fake provider. It always answers ANALOG for the first candidate listed and
    NOT_ANALOG for the rest -- an honestly mediocre checker that exercises block -> prompt -> call
    -> parse -> decide -> lift end to end with no key. It is NOT a baseline; evals/baseline.py is."""
    marker = "CANDIDATES ("
    block_ = user.split(marker, 1)[-1] if marker in user else ""
    ids = [ln.split(" -- ")[0].strip() for ln in block_.splitlines() if ln.strip().startswith("EVT-")]
    verdicts = [{"event_id": eid, "verdict": "ANALOG" if i == 0 else "NOT_ANALOG",
               "reason": "stub"} for i, eid in enumerate(ids)]
...
def main():
    ...
    for i, req in enumerate(requests, 1):
        ...
        per_request_records.append({
            ...
            "analog_event_ids": r["analog_event_ids"], "draft": r["draft"], "parsed": r["parsed"],
        ...
        print("  %2d/%d  %-10s cands=%2d  analogs=%-2d  decision=%-20s %s"
             % (i, len(requests), r["request_id"], len(r["candidates"]),
                len(r["analog_event_ids"]), r["draft"]["decision"],
    ...
    from src import lift as L
    dtally = L.tally(drafts)
    ...
    print("%-26s %s / %s" % ("blocking recall", bstats["true_analog_pairs_surviving"],
                             bstats["true_analog_pairs"]))
    print("%-26s %s" % ("draft ready", dtally["counts"]["draft_ready"]))
    print("%-26s %s" % ("insufficient precedent", dtally["counts"]["insufficient_precedent"]))
