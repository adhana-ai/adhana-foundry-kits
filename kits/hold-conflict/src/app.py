...
ONE RECORDS-DISPOSITION REVIEW, ONE EXTRACTION, on your machine, with your key. It is the proof
that this runs on a laptop; it is NOT the published dashboard, which lives on the Foundry site
from committed run records.

⚠︎ IT PROPOSES AND IT NEVER DISPOSES. There is no destroy button, no approve button and no write
path of any kind. The panel says whether a series may be PUT IN FRONT OF a records officer, and
the flag underneath asks for a series to be pulled back OUT of a destruction queue. Both are
...
# ⚑ EVERY KIT PICKS ITS OWN PORT SO TWO CAN RUN AT ONCE. This kit takes 8842 rather than the next
# number after its predecessor: it was built alongside eight sibling kits in one sprint, all
# picking "the next free one" from the same list at the same time, which is a collision waiting
# to happen. 8842 is derived from this kit's own use-case id (UC042) and cannot collide with a
# neighbour that has not been written yet.
...
def main():
    ...
    print("hold-conflict UI  ->  http://127.0.0.1:%d" % a.port)
    print("  reviews: %d   fields: %d   review date: %s   API_KEY: %s"
          % (len(EX.documents()), len(EX.load_fields()), EX.AS_OF,
