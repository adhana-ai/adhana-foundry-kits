...
class AdapterError(RuntimeError):
    ...
    distinction was added to this file after it cost a SIBLING kit two documents of a paid run, and
    this kit ships it from birth rather than re-learning it.
    """
...
def _post(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise AdapterError("%s %s: %s" % (e.code, e.reason, e.read().decode("utf-8", "replace")),
                           status=e.code)
    # ⚑ A DROPPED CONNECTION IS NOT A FAILED DOCUMENT EITHER — written into this file after a
    # SIBLING kit's first paid run of 55 lost TWO documents, one to `<urlopen error [Errno 60]
    # Operation timed out>` and one to `[Errno 54] Connection reset by peer`. This kit ships the
    # clause from its first commit; nothing here has yet SEEN a transport failure, which is the
    # honest state and not a claim that the branch has been exercised live.
    #
...
def openai_compatible(cfg, system, user, max_tokens, thinking=None):
    ...
    # from. Sibling kits in this series have measured provider-side reasoning eating three quarters
    # of every reply's output budget, and this is the knob that would test it here. THIS KIT'S
    # HARNESS NEVER SENDS IT -- every published run left reasoning at the provider's default, and
    # the result files record `thinking: null` so a future run that DOES send it cannot be quietly
    ...
    return {"text": choice["message"]["content"],
            ...
            # ⚑ THE PROVIDER SAYS WHY IT STOPPED — RECORD IT. A reply cut off at the ceiling and a
            # reply the model chose to end are the same empty cell to a scorer and completely
            # different defects to a person. Sibling kits in this series published three runs whose
            # "failures" were nothing but a cap set from a smaller corpus, with the API answering
            # half the question in this field the whole time and nothing reading it.
            "finish_reason": choice.get("finish_reason"),
            # ⚑ WHERE THE BUDGET WENT. `finish_reason` says the reply was cut off; this says what
            # ate it. On this kit's own list-shaped reply the question is live rather than
            # hypothetical -- a worksheet of five to seven lines is a lot of JSON, and how much of
            # the budget goes to reasoning instead is the thing evals/run.py records. Absent on
            # providers that do not report it, which is the third state and not a zero.
...
def complete(cfg, system, user, max_tokens=1024, thinking=None):
    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            # ⚑ `or exc.transport` IS THE HALF THAT IS EASY TO MISS. A reset connection and a
            # socket timeout have no status to test, so `exc.status not in TRANSIENT` is True for
            # both and a status-only loop re-raises on the first attempt. Two documents of a paid
            # 55 were lost to exactly that in a sibling kit before this clause existed.
