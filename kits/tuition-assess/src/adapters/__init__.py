...
def openai_compatible(cfg, system, user, max_tokens, thinking=None):
    """Covers every provider that speaks the OpenAI chat-completions shape -- OpenAI itself, Groq,
    Together, Mistral, xAI, Fireworks, and any local server (Ollama, LM Studio, vLLM), along with
    several hosted providers not listed here. BASE_URL is what selects between them, which is why
    it is in .env rather than in here.
    ...
    # from. This kit has exactly one record where the knob would matter: STU-0050 spent all 4,000
    # output tokens on the fast tier and returned an EMPTY `text`, which is what a reasoning pass
    # nobody asked for looks like from the outside. It is NOT the answer yet -- testing it needs
    # its own run, and neither published run used it, so both stay comparable to a future one that
    # also omits it.
    ...
    return {"text": choice["message"]["content"],
            ...
            # ⚑ THE PROVIDER SAYS WHY IT STOPPED — RECORD IT. It is what turned this kit's one
            # fast-tier failure from a mystery into a sentence: STU-0050 came back with an empty
            # `text` and `finish_reason` "length" at exactly 4,000 output tokens, so the reply was
            # cut off rather than refused. evals/run.py reads this field to say so in the run
            # record, and it is why `output_tokens_max` is published on every run.
            "finish_reason": choice.get("finish_reason"),
            # ⚑ WHERE THE BUDGET WENT. `finish_reason` says the reply was cut off; this says what
            # ate it. Absent on providers that do not report it, which is the third state and not a
            # zero. ⚠︎ THIS KIT'S RUN RECORDS DO NOT CARRY IT -- evals/run.py never wrote the field
            # -- so the truncation above is published as "4,000 tokens produced no text", which is
            # what was measured, rather than as a reasoning breakdown nobody here captured.
...
# The one shape this kit will send, taken verbatim from the runtime provider's own documented
# request example for the model this kit was run on. Named rather than spelled inline at the call
# site so there is exactly one place a provider's documented field lives, and so a run record can
# say which one it used.
#
# ⚠︎ NEITHER RUN IN results/ USED IT. `thinking` is passed through only when a caller asks for it,
# and neither scored run did -- so both requests are byte-identical to what they would have been
# before this parameter existed, and a future run that also omits it stays comparable to them.
