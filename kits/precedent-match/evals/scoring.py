...
Precision and recall are over the ANALOG label, at the pair level (one request, one candidate).
`false_merge` (a candidate wrongly counted into the lift estimate) is the expensive direction --
same shape data-match's decide.py states about its own false merges -- because it corrupts a
number a planner acts on, invisibly. `missed_match` is recoverable: a real precedent just does not
get used.
"""
...
def score_request(per_candidate, gold_by_pair, request_id, expects_verdict=True):
    ...
    for c in per_candidate:
        label = gold_by_pair.get((request_id, c["event_id"]))
        if label is None:
            continue
...
def aggregate(all_rows):
    ...
    true analog, a real precedent silently went unused (the same cost as missed_match); if it was
    not, no harm was done (the same as apart_correct). `counts` itself still reports the raw
    ...
    no_verdict_analog = sum(1 for r in all_rows
                            if r["outcome"] == "no_verdict" and r["label"] == "analog")
    no_verdict_not = counts["no_verdict"] - no_verdict_analog

    tp = counts["merged_correct"]
    fp = counts["false_merge"]
    fn = counts["missed_match"] + no_verdict_analog
