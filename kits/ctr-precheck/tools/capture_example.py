...
"""Capture one reply VERBATIM, for the LLM lens's `raw_response`. SPENDS ONE CALL.
    ...
    python3 tools/capture_example.py --doc QCP-0014

⚑ WHY A FRESH CALL RATHER THAN A REPLY FROM THE SCORED RUN. evals/run.py records only the FIRST
pack's raw text, because storing 56 full replies in every result file makes the run record
...
though it were a 57th scored pack.
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--doc", default="QCP-0014")
    ap.add_argument("--beside", default="r001-ctr-precheck")
    ...
    out = {
        ...
        "note": ("A fresh single call replayed after the scored run, to capture one reply "
                 "verbatim. It is NOT a 57th scored pack and is not counted in any denominator."),
        "model": cfg.get("model"),
        ...
        "needs_recompute": r.get("needs_recompute"),
        "recomputed_defects": r.get("recomputed_defects"),
        "recomputed_reasons": r.get("recomputed_reasons"),
        "document_text": text,
    }
    ...
    print("\ninput %s  output %s  finish %s" % (out["input_tokens"], out["output_tokens"],
                                                out["finish_reason"]))
    print("needs_recompute %r   recomputed %r" % (out["needs_recompute"],
                                                  out["recomputed_defects"]))
