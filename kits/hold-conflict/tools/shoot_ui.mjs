...
try {
  ...
  // ⚠︎ 1200 HIGH, TALLER THAN THE SIBLING KIT THIS WAS MODELLED ON. This kit's payoff is the
  // ROUTING panel under the field table -- TWELVE field rows plus four routed rows, plus the
  // scope warning above the controls -- and at 1040 the 'Needs a records-officer follow-up' row
  // falls below the fold, which is the one row the whole page exists to show.
  ...
  if (LIVE) {
    // RDS-0004 exercises every moving part at once, and is the sharpest record in the corpus:
    // a hold that COVERS this series exactly and reads `released`, plus a second, ACTIVE registry
    // line whose scope is only "continues the scope of AH-2023-234". Reading the released line
    // and stopping answers "eligible" and is wrong. Its own retention expired 2025-12, there is
    // no overlapping series, the officer's note reads "Routine end-of-retention item. Nothing
    // outstanding on this series." (the wrong register -- this series is FROZEN), and it is
    // already `queued`, so the pure-code flag fires.
    await page.select('#doc', 'RDS-0004')
    await sleep(300)
  }
