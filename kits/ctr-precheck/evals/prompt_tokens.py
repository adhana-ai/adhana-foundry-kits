"""Measure the prompt split in TOKENS, not characters. Spends a few calls at max_tokens=1.
    ...
    python -m evals.prompt_tokens --run-id p001-ctr-precheck --doc QCP-0001 --yes
...
⚑ THIS KIT HAS FOUR PARTS, NOT THREE, AND THE EXTRA ONE IS THE RULEBOOK. The threshold, the window,
the code table and the element list are all this kit's own inventions, so they have to travel in the
call -- which makes "how much of every call is the rulebook" a question a reader should be able to
answer with a number rather than a guess.
"""
import argparse
import json
import os
...
def prefixes(doc_text, fields):
    ...
    return system, [("", "chat protocol and system prompt"),
                    (a, "filing rulebook"),
                    (b, "field schema"),
                    (full_user, "qc pack sections")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--run-id", required=True)
    ap.add_argument("--doc", default="QCP-0001")
