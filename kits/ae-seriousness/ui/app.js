...
function draw(fields) {
  ...
  // NOT AN ERROR COUNT. narrative_severity_word is legitimately null on reports that use no
  // severity word, and returning null there is the correct answer — see data/SOURCES.md.
...
function drawComputed(fields, flag) {
  const val = (n) => (fields && fields[n] && fields[n].value) || null;
  const serious = val("is_serious");
  const cause = val("causality_assessment");
  $("c-serious").textContent = serious === null ? "—"
    : (serious === "yes" ? "YES — a seriousness criterion was met" : "no — no criterion met");
  $("c-cause").textContent = cause === null ? "—" : cause;
  $("c-flag").textContent = (flag === null || flag === undefined)
    ? "not computed — a required input was missing or out of vocabulary"
    : (flag ? "YES — route for expedited case-processor review" : "no");
}
...
async function go() {
  ...
  try {
    ...
    draw(r.fields);
    drawComputed(r.fields, r.needs_review);
  } finally { $("go").disabled = false; $("go").textContent = "Extract"; }
}

$("go").addEventListener("click", go);
$("doc").addEventListener("change", () => { draw(null); drawComputed(null, null); show(); });
load();
